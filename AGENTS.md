...
- Etsy 상점 태그라인을 `Printable journals for a dog's golden years`로 추가하고 공개 반영 확인
- Etsy 상점 공지에 93페이지 디지털 다운로드, 홈 프린트, 구매 전 문의 안내를 추가하고 공개 반영 확인
- Etsy FAQ에 파일 형식, 실물 배송 없음, 인쇄·개인 사용 범위, 비수의학적 자료임을 명시하고 공개 반영 확인
- Etsy 디지털 상품 기본 반품 정책(`No returns or exchanges`, 문제 발생 시 판매자 문의)이 활성 상품에 적용된 상태 확인
- Beehiiv 공개 구독 폼과 `/subscribe` 모달은 정상 로드됨
- Beehiiv 첫 공개 글 `Welcome to Gomi’s Golden Days`를 Web only로 발행하고 홈페이지 및 본문 공개 노출 확인: `https://gomisgoldendays.beehiiv.com/p/welcome-to-gomis-golden-days`
- 첫 글은 이메일을 발송하지 않았으며, Beehiiv 게시 설정 화면에는 `4 recipients`가 표시됨
- Beehiiv `/archive` 404 원인은 Archive 기본 페이지가 Draft 상태였기 때문이며, 해당 페이지만 live 게시한 뒤 홈페이지 링크 이동·게시글 노출·데스크톱·모바일 공개 화면을 확인함
...
4. 저널 상품 페이지와 신뢰 요소 1차 개선 완료 — Etsy 태그라인·공지·FAQ·디지털 상품 정책 공개 확인
5. Beehiiv 환영메일·체크리스트 전달·첫 공개 글·Archive 페이지 게시 완료 — 후속 이메일 흐름만 추후 점검
